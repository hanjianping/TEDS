function gotoDemandPage(num){
	 var trainId=$("#trainId").val();
      var firstDate=$("#firstDate").val();
	  var lastDate =$("#lastDate").val();
	if(num>0){
		window.location.href='Video_history.action?pageCode='+num+'&trainId='+trainId+'&firstDate='+firstDate+'&lastDate='+lastDate;
	}
}
 function searchTrain(){
	  var trainId=$("#trainId").val();
	  var firstDate=$("#firstDate").val();
	  var lastDate =$("#lastDate").val();
  	window.location.href='Video_history.action?trainId='+trainId+'&firstDate='+firstDate+'&lastDate='+lastDate;
}
 function checkEndDate(){
	 var firstDate=$("#firstDate").val();
	 	var lastDate =$("#lastDate").val();
	 	var beginDate=new Date(firstDate.replace(/^(\d{4})(\d{2})(\d{2})$/,"$1/$2/$3"));
	 	var endDate=new Date(lastDate.replace(/^(\d{4})(\d{2})(\d{2})$/,"$1/$2/$3"));
	 	if(beginDate>endDate){
	 		$("#firstDate").val('');
	 		alert("结束时间必须大于起始时间");
	 	}
}
function checkBeforeDate(){
	 var firstDate=$("#firstDate").val();
	 	var lastDate =$("#lastDate").val();
	 	var beginDate=new Date(firstDate.replace(/^(\d{4})(\d{2})(\d{2})$/,"$1/$2/$3"));
	 	var endDate=new Date(lastDate.replace(/^(\d{4})(\d{2})(\d{2})$/,"$1/$2/$3"));
	 	if(beginDate>endDate){
	 		$("#lastDate").val('');
	 		alert("起始时间必须小于结束时间");
	 	}
}
function video(src){
	$('#video').modal();
	$("#my_video").attr("src",src);
}
